...
const SignUp = () => {
	...
	const [ is_doctor, setIsDoctor ] = useState<boolean>(false);
	...
	const handleRegistration = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		try {
			const result = await signUp({ username: login, email, password, is_doctor });
			if (result.error ) {
			...
			<form method="post" onSubmit={handleRegistration}>
				...
				<div className="form-group">
					<label>
						...
						/>
						<span style={{ marginLeft: '8px' }}>Зарегистрироваться как врач</span>
					</label>
				</div>
